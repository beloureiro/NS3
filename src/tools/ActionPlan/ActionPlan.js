...
import { ChevronLeft, Globe, Check, PlusCircle, AlertCircle, Trash2 } from 'lucide-react';
                  ...
                  {plans.map((plan, index) => (
                    <tr key={index}>
                      ...
                      {t.questions.map((q) => (
                        ...
                        >
                          {editingField && editingField.index === index && editingField.field === q.key ? (
                            (q.key === 'urgency' || q.key === 'importance') ? (
                              <select
                                defaultValue={plan[q.key]}
                                onBlur={(e) => handleEditSave(e, index, q.key)}
                                onChange={(e) => handleEditSave(e, index, q.key)}
                                autoFocus
                                className="bg-[#111111] text-gray-300 border border-[#00ff9d] focus:border-[#00ff9d] focus:ring-[#00ff9d] rounded-md p-1 w-full"
                              >
                                {[1, 3, 5].map(value => (
                                  <option key={value} value={value} style={{color: 'white', backgroundColor: '#1a1a1a'}}>
                                    {value === 1 ? t.low : value === 3 ? t.medium : t.high}
                                  </option>
                                ))}
                              </select>
                            ) : (
                              <input
                                ...
                                onBlur={(e) => handleEditSave(e, index, q.key)}
                                onKeyPress={(e) => {
                                  if (e.key === 'Enter') handleEditSave(e, index, q.key);
                                }}
                                autoFocus
                                className="bg-[#111111] text-gray-300 border border-[#00ff9d] focus:border-[#00ff9d] focus:ring-[#00ff9d] rounded-md p-1 w-full"
                              />
